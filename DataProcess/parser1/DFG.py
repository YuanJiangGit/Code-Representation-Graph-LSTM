...
def DFG_java(root_node,index_to_code,states):
    ...
    do_first_statement=[]    
    states=states.copy()
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment': # 叶子节点
        idx,code=index_to_code[(root_node.start_point,root_node.end_point)]
        if root_node.type==code: # 如果类型等于code，代表它不是重要的叶子节点 （e.g., '{','while'）
            return [],states
        elif code in states: #（2）如果当前节点对应的code 在 states，那么当前节点会和states中的节点（对应的id）建立数据流链接关系
            return [(code,idx,'comesFrom',[code],states[code].copy())],states
        else:
            if root_node.type=='identifier': # （1）如果当前节点是定义节点，例如 (String) service, serve 会被加入到states（code<--->id）
                states[code]=[idx]
            return [(code,idx,'comesFrom',[],[])],states # （3）如果叶子节点不在states中，代表它不指向任何节点，指向位置为空
    elif root_node.type in def_statement: # (String) filename = this.baseDirectory + service + ".json"
        name=root_node.child_by_field_name('name') # (等式左边 filename) <Node kind=identifier, start_point=(1, 15), end_point=(1, 23)>
        value=root_node.child_by_field_name('value') # (等式右边的子树)  <Node kind=binary_expression, start_point=(1, 26), end_point=(1, 64)>
        DFG=[]
        if value is None:
            indexs=tree_to_variable_index(name,index_to_code)
            for index in indexs:
            ...
            DFG+=temp # temp是子树的DFG，将子树的DFG加入到总的DFG中
            for index1 in name_indexs:
                idx1,code1=index_to_code[index1]
                for index2 in value_indexs:   # 分别对应等式左边的所有变量 （e.g., baseDirectory, service, ".json"）的位置
                    idx2,code2=index_to_code[index2]
                    DFG.append((code1,idx1,'comesFrom',[code2],[idx2]))  # 将左边的变量都要和右边的变量建立联系
                states[code1]=[idx1]  # 左边是变量定义，(e.g.,String filename),所以变量定义应该加到states
            return sorted(DFG,key=lambda x:x[1]),states # 按照idx1进行排序
    elif root_node.type in assignment:
        left_nodes=root_node.child_by_field_name('left')
        right_nodes=root_node.child_by_field_name('right')
        DFG=[]
        ...
        for i in range(2):
            for child in root_node.children:
                ...
                DFG+=temp    
        dic={}
        for x in DFG: # (e.g.,  ('s', 52, 'comesFrom', ['s'], [23]),), 有可能's', 52, 'comesFrom'有多个来源，将多个来源进行合并，合并之后可能是('s', 52, 'comesFrom', ['s'，'b'], [23, 24])
            if (x[0],x[1],x[2]) not in dic:
                dic[(x[0],x[1],x[2])]=[x[3],x[4]] # {('s', 52, 'comesFrom'): [['s'], [23]]}
            else:
                dic[(x[0],x[1],x[2])][0]=list(set(dic[(x[0],x[1],x[2])][0]+x[3])) # 左边:dic[(x[0],x[1],x[2])][0] 是s， 右边：dic[(x[0],x[1],x[2])][0]+x[3] 是[s,s]
                dic[(x[0],x[1],x[2])][1]=sorted(list(set(dic[(x[0],x[1],x[2])][1]+x[4])))
        DFG=[(x[0],x[1],x[2],y[0],y[1]) for x,y in sorted(dic.items(),key=lambda t:t[0][1])]
        return sorted(DFG,key=lambda x:x[1]),states        
    else:
